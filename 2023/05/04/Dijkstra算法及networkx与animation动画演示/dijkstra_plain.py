...
class Graph:
    ...
    def dijkstra(self, start):
        ...
        while (current_vertex := self.get_min_dist_vertex(distances, visited)) != -1:
            # 置为已访问
            visited[current_vertex] = True
            for neighbor in range(self.vertex_num):
                if self.edges[current_vertex][neighbor] != -1:
                    dist_un = self.edges[current_vertex][neighbor]
                    # u的邻接节点中未访问的节点v，松弛v的距离
                    if not visited[neighbor]:
                        dist = distances[current_vertex] + dist_un
...
class Draw:
    ...
    def draw_animation_process(self, fig, ax, n, visited_list, distances_list, previous_vertex_list):
        ani = animation.FuncAnimation(fig, self.animation_process_func, frames=n + 1,
